...
class SuperAI(plus.AI):
    ...
    def GetNearestEnemy(self):
        ...
        for enemy in enemies:
            ...
            weighted_dist = dist
            if not plus.isHuman(enemy): weighted_dist += 0
    ...
    # Modified waypoint navigation for implementation into race arena py's
               
    def Drive2Waypoints(self, waypoints, in_reverse = False):
        throttle = 0
        found = False
        a = Arenas.currentArena
        #point = 0
        ...
        while len(waypoints) > 0 and not found:
            ...
            dir = 1
            if len(waypoints) > 1:
                clear_path = self.IsStraightPathClear(self.GetLocation(), waypoints[1])
            else:
                clear_path = 0
            #a.tbox0.setText(str(waypoints))
            #a.tbox1.setText(str(dist))
            #a.tbox2.setText(str(clear_path))
            
            if in_reverse: dir = -1
            
            if clear_path or dist < 2:
                waypoints.pop(0)
                #point += 1
            else:
                # drive to this point
